...
class ExcelReporter {
  ...
  async onRunComplete(contexts, results) {
    ...
    titleCell.value = 'CollabRoom Web & Mobile Load Test Suite Summary';
    ...
    summarySheet.addRow(['Target Environment:', 'Web & Mobile Performance Validation', '']);
    ...
    const xlsxPath = path.join(reportDir, 'load-report.xlsx');
    await workbook.xlsx.writeFile(xlsxPath);
    console.log(`Excel report generated at: ${xlsxPath}`);

    // --- CSV report ---
    const csvSummaryRows = [
      ['CollabRoom Web & Mobile Load Test Suite Summary'],
      ['Report Generation Date:', new Date().toLocaleString()],
      ['Target Environment:', 'Web & Mobile Performance Validation'],
    ...
    const csvPath = path.join(reportDir, 'load-report.csv');
