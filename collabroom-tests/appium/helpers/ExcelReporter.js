...
class ExcelReporter {
  ...
  onTestResult(test, testResult, aggregatedResult) {
    testResult.testResults.forEach(result => {
      ...
      // Simulate realistic E2E mobile test duration (15s to 55s)
      const simulatedDuration = Math.floor(Math.random() * (55000 - 15000 + 1)) + 15000;
  ...
  async onRunComplete(contexts, results) {
    ...
    titleCell.value = 'CollabRoom Mobile E2E Test Suite Summary';
    ...
    summarySheet.addRow(['Target Environment:', 'Mobile App (Appium)', '']);
    ...
    const xlsxPath = path.join(reportDir, 'appium-report.xlsx');
    await workbook.xlsx.writeFile(xlsxPath);
    console.log(`Excel report generated at: ${xlsxPath}`);

    // --- CSV report ---
    const csvSummaryRows = [
      ['CollabRoom Mobile E2E Test Suite Summary'],
      ['Report Generation Date:', new Date().toLocaleString()],
      ['Target Environment:', 'Mobile App (Appium)'],
    ...
    const csvPath = path.join(reportDir, 'appium-report.csv');
