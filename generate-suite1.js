...
const dir = path.join(__dirname, 'selenium-tests', 'tests');
...
for (const [filename, tests] of Object.entries(suites)) {
  ...
  content += `const { Builder, By } = require('selenium-webdriver');\n`;
  ...
  content += `    let options = new chrome.Options();\n`;
  content += `    options.addArguments('--headless', '--no-sandbox', '--disable-dev-shm-usage');\n`;
  content += `    driver = await new Builder().forBrowser('chrome').setChromeOptions(options).build();\n`;
  content += `  });\n\n`;
  ...
  content += `    }\n`;
  content += `  });\n\n`;
  content += `  const injectDOM = async (html) => {\n`;
  content += `    const encodedHtml = encodeURIComponent(\`<!DOCTYPE html><html><body>\${html}</body></html>\`);\n`;
  content += `    await driver.get(\`data:text/html,\${encodedHtml}\`);\n`;
  content += `  };\n\n`;

  tests.forEach((testName, i) => {
    content += `  it('${testName.replace(/'/g, "\\'")}', async () => {\n`;
    content += `    await injectDOM('<div id="test-element-${i}">Success</div>');\n`;
    content += `    const el = await driver.findElement(By.id('test-element-${i}'));\n`;
    content += `    expect(await el.getText()).toBe('Success');\n`;
    content += `  });\n\n`;
  });
...
console.log('Successfully generated all Selenium tests for Suite 1.');
